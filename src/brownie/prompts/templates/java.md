...
- Entry points: Spring Boot main classes, `Application.java`, `Main.java`.
- Frameworks: Spring MVC, Spring WebFlux, Jakarta EE, Micronaut, Quarkus.
- Structure: `controller`, `service`, `repository`, `model`, `domain` packages.
- Data models: POJOs, records, JPA entities, DTOs.
...
| -------- | ------------ | --------------- |
| **Project Intent & Business Frame** | Main application class, config files, README, build files (`pom.xml`, `build.gradle`) | Purpose, commands, flags, exit codes, business goals |
| **Domain Landscape** | Domain models, enums, DTOs, entity classes | Core concepts, entity relationships, boundaries |
| **Canonical Data Model** | POJOs/records, config properties, persistence models | Field names, types, defaults, JSON schemas |
| **Service & Capability Map** | Controllers, services, repositories, module entrypoints | Module responsibilities, public functions, dependencies |
| **Architectural Guardrails** | Validation annotations, exception handlers, limit constants | Constraints, thresholds, validation rules, error paths |
| **API & Integration Contracts** | Controllers, HTTP clients, SDK usage, messaging | Provider interfaces, external dependencies, contracts |
| **User Journey & UI Intent** | CLI usage, endpoints, logs/output formatting, user-facing messages | User flows, commands, output artifacts, error messages |
...
```text
**Evidence:** `<file>.java:<line>` - `<relevant code snippet>`
```

Or inline: `The <MethodName> method does X (path/to/file.java:45-77).`
...
Use tree notation for classes/records and config structures. Format example:
...
3. **Concrete symbols:** Use actual class/method/type names, not descriptions.
