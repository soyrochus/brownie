...
- Entry points: `main.*`, `cli.*`, `index.*`, `server.*`, `app.*`.
- Structure: top-level modules, `src/`, `app/`, `services/`, `packages/`.
- Data models: classes/structs, schemas, type definitions, configuration objects.
...
| -------- | ------------ | --------------- |
| **Project Intent & Business Frame** | Entry modules, config files, README, build manifests | Purpose, commands, flags, exit codes, business goals |
| **Domain Landscape** | Domain models, type definitions, enums | Core concepts, entity relationships, boundaries |
| **Canonical Data Model** | Data structures, config objects, persistence modules | Field names, types, defaults, JSON schemas |
| **Service & Capability Map** | Module entrypoints, service layers, orchestration flows | Module responsibilities, public functions, dependencies |
| **Architectural Guardrails** | Config defaults, validation logic, error handlers, limit constants | Constraints, thresholds, validation rules, error paths |
| **API & Integration Contracts** | External imports, SDK usage, HTTP clients, tool interfaces | Provider interfaces, external dependencies, contracts |
| **User Journey & UI Intent** | CLI flows, UI entrypoints, user-facing output | User flows, commands, output artifacts, error messages |
...
Or inline: `The <symbol> does X (path/to/file:45-77).`
...
Use tree notation for data/config structures. Format example:
...
3. **Concrete symbols:** Use actual symbol/type names, not descriptions.
