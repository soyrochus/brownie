...
- Frameworks: net/http, Gin, Echo, Chi, Fiber.
- Structure: packages under `internal/`, `pkg/`, `cmd/`.
- Data models: structs, type aliases, SQL models, JSON tags.
...
| -------- | ------------ | --------------- |
| **Project Intent & Business Frame** | `main.go`, `cmd/*/main.go`, config files, README | Purpose, commands, flags, exit codes, business goals |
| **Domain Landscape** | Domain packages, type definitions, enums/consts | Core concepts, entity relationships, boundaries |
| **Canonical Data Model** | Structs, config structs, persistence layers | Field names, types, defaults, JSON schemas |
| **Service & Capability Map** | Package entrypoints, service interfaces, orchestration functions | Module responsibilities, public functions, dependencies |
...
| **API & Integration Contracts** | HTTP handlers, clients, SDK usage, message queues | Provider interfaces, external dependencies, contracts |
| **User Journey & UI Intent** | CLI parsing, logs/output formatting, user-facing messages | User flows, commands, output artifacts, error messages |
...
```text
**Evidence:** `<file>.go:<line>` - `<relevant code snippet>`
```

Or inline: `The <funcName> function does X (path/to/file.go:45-77).`
...
Use tree notation for structs and config structures. Format example:
...
3. **Concrete symbols:** Use actual function/type/struct names, not descriptions.
