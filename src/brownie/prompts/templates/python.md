...
- Frameworks: FastAPI, Flask, Django, Typer, Click, argparse.
- Structure: packages in `src/`, `app/`, or top-level modules.
- Data models: dataclasses, Pydantic models, SQLAlchemy/Django models, enums.
...
| -------- | ------------ | --------------- |
| **Project Intent & Business Frame** | CLI entry (`cli.py`, `__main__.py`), orchestration modules, `pyproject.toml`, README | Purpose, commands, flags, exit codes, business goals |
| **Domain Landscape** | Data model files, `models.py`, domain classes, enums | Core concepts, entity relationships, boundaries |
| **Canonical Data Model** | Dataclasses, Pydantic models, config classes, cache/persistence modules | Field names, types, defaults, JSON schemas |
| **Service & Capability Map** | Module `__init__.py` files, service classes, orchestration functions | Module responsibilities, public functions, dependencies |
...
| **API & Integration Contracts** | External imports, SDK usage, HTTP clients, tool definitions | Provider interfaces, external dependencies, contracts |
| **User Journey & UI Intent** | CLI definitions (argparse/click/typer), feedback classes, output formatters | User flows, commands, output modes, error messages |
...
```text
**Evidence:** `<file>.py:<line>` - `<relevant code snippet>`
```

Or inline: `The <function_name> function does X (path/to/file.py:45-77).`
...
|-------|------|---------|----------|
| `<field_name>` | `<type>` | `<default>` | `<file>:<line>` |
```

### Code Structure Trees (Required for Data Models)

Use tree notation for dataclasses and config structures. Format example:
...
| Canonical Data Model | 3 | 5 | Code tree per major dataclass, field tables |
...
3. **Concrete symbols:** Use actual function/class/enum names, not descriptions.
