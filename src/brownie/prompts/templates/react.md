...
- Entry points: `src/index.*`, `src/main.*`, `App.*`, `pages/_app.*`, `app/layout.*`.
- Frameworks: React, Next.js, Remix, Gatsby, React Router.
- Structure: `src/`, `app/`, `pages/`, `components/`, `routes/`, `layouts/`, `hooks/`.
- UI models: component props, client state shapes, form schemas, API response types.
...
| -------- | ------------ | --------------- |
| **Project Intent & Business Frame** | Entry modules (`src/main.*`, `App.*`), router setup, README, `package.json` | Purpose, commands, build/run scripts, business goals |
| **Domain Landscape** | Domain types, API response models, state schemas, form models | Core concepts, entity relationships, boundaries |
| **Canonical Data Model** | Component props, state types, validation schemas, config objects | Field names, types, defaults, JSON schemas |
| **Service & Capability Map** | Module `index.*` files, service hooks, data access utilities | Module responsibilities, public functions, dependencies |
| **Architectural Guardrails** | Config defaults, validation utilities, error boundaries, limit constants | Constraints, thresholds, validation rules, error paths |
| **API & Integration Contracts** | Data fetching hooks, HTTP clients (`fetch`, axios), SDK usage | Provider interfaces, external dependencies, contracts |
| **User Journey & UI Intent** | Route definitions, page components, layouts, navigation, user-facing copy | User flows, screens, output artifacts, error messages |
...
```text
**Evidence:** `<file>.tsx:<line>` - `<relevant code snippet>`
```

Or inline: `The <Component> renders X (path/to/file.tsx:45-77).`
...
Use tree notation for props/state/config structures. Format example:
...
| User Journey & UI Intent | 1 | 5 | User flow diagram or step sequence |
...
3. **Concrete symbols:** Use actual component/hook/function/type names, not descriptions.
