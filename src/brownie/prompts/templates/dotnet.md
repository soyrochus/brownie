...
- Entry points: `Program.cs`, `Startup.cs`, `appsettings.json`.
- Frameworks: ASP.NET Core, minimal APIs, MVC, gRPC.
- Structure: `Controllers/`, `Services/`, `Models/`, `Domain/`, `Infrastructure/`.
- Data models: classes/records, DTOs, EF Core entities and DbContext.
...
| -------- | ------------ | --------------- |
| **Project Intent & Business Frame** | `Program.cs`, `Startup.cs`, `appsettings.json`, README | Purpose, commands, flags, exit codes, business goals |
| **Domain Landscape** | Domain models, enums, DTOs, entity classes | Core concepts, entity relationships, boundaries |
| **Canonical Data Model** | Records/classes, config options, EF Core models | Field names, types, defaults, JSON schemas |
| **Service & Capability Map** | Controllers, services, modules, DI registrations | Module responsibilities, public functions, dependencies |
| **Architectural Guardrails** | Validation attributes, middleware, error handlers, limit constants | Constraints, thresholds, validation rules, error paths |
| **API & Integration Contracts** | Controllers, minimal API maps, SDK usage, HTTP clients | Provider interfaces, external dependencies, contracts |
...
```text
**Evidence:** `<file>.cs:<line>` - `<relevant code snippet>`
```

Or inline: `The <MethodName> method does X (path/to/file.cs:45-77).`
...
Use tree notation for records/classes and config structures. Format example:
