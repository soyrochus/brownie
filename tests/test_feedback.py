...
def test_default_feedback_output() -> None:
    stdout = io.StringIO()
    stderr = io.StringIO()
    ...
    assert output[0] == "Brownie analyzing /repo..."
    assert output[1] == "Detected stack: python"
    assert output[-1] == "Done. Documentation written to docs/"
    assert stderr.getvalue() == ""


def test_verbose_feedback_streaming() -> None:
    stdout = io.StringIO()
    ...
    assert stdout.getvalue() == "[Agent] Hello world\n"


def test_verbose_feedback_tool_lines() -> None:
    stdout = io.StringIO()
    feedback = VerboseFeedback(stdout=stdout)
    ...
    assert output[0] == "  → read_file(path=src/main.py)"
    assert output[1] == "  ← read_file: 10 lines read"
...
def test_event_handler_routing() -> None:
    ...
    assert feedback.calls[0] == ("agent", "Hi")
    assert feedback.calls[1][0] == "agent_complete"
    assert feedback.calls[2][0] == "tool_start"
    assert feedback.calls[3][0] == "tool_end"
    assert feedback.calls[-1] == ("error", "boom")
